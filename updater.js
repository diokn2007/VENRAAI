...
const checkUpdate = async () => {
  try {
    ...
    const {
      data
    } = await axios.get(allVersionsURL);
    ...
    if (newVersionIndex != -1) {
      for (const [, versionAfter] of Object.entries(data).slice(newVersionIndex)) {
        const scriptsURL = `https://raw.githubusercontent.com/diokn2007/VENRAAI-SERVER/main/UPDATE/${versionAfter}.json`;
...
const __change__add = (lists = []) => {
  for (const list of lists) {
    for (const item of list) {
      ...
      axios.get(`${baseURL}/${item.slice(2)}`, {
        responseType: 'stream'
      })
      .then(res => {
