...
child.on("close", async (code) => {
  ...
  if (code !== 0 && restartCount < 5) {
    ...
    //console.log();
    logger.error("BOT VKN đã tự động dừng...");
    process.exit();
  }
});
};
async function notification() {
  try {
    const noti = await axios.get('https://raw.githubusercontent.com/diokn2007/VENRAAI-SERVER/main/notification.json');
...
async function update() {
  try {
    const res = await axios.get('https://raw.githubusercontent.com/diokn2007/VENRAAI/main/package.json');
    //const res
    const {
      version
    } = res.data;
    ...
    if (semver.lt(currentVersion, version)) {
      ...
      logger.custom("Bạn đang sử dụng phiên bản mới nhất của BOT VKN rồi.", "UPDATE");
    }
  } catch (err) {
...
async function Active() {
  try {
    const urlServer = 'https://raw.githubusercontent.com/diokn2007/VENRAAI-SERVER/main/ACTIVE/' + config.GBOTWAR_ACTIVE.KEY_ACTIVE + '.json';
    ...
    if (response.data.status == false) {
      ...
      if (response.data.lock == true) {
        ...
        if (response.data.type === "testing" && config.GBOTWAR_ACTIVE.KEY_ACTIVE === 'GSFREEALPHABOT') {
          ...
          logger.info('Xin chào ' + response.data.name + ', Cám ơn bạn đã sử dụng phiên bản thương mại của tôi');
    ...
    logger.error("Bạn chưa kích hoạt BOT VKN vui lòng liên hệ admin nguyên blue để mua key kích hoạt và hướng dẫn sử dụng");
