...
const config = {
  ...
  permissions: [2],
  extra: {
    "time": 10000//time mặc định spam không nên thay đổi nếu đéo muốn bị khóa mõm
  }
}
if (!global.Spamv2) {
  global.Spamv2 = [];
...
async function Running( {
  ...
  const FileCache = readFileSync(resolvePath(global.nguyenblue.Lyrics, "spam.txt"));
  let select = args[0]?.toLowerCase();
  let contentAdd = args.slice(1);
  switch (select) {
    case 'add':
      const content = contentAdd.join(" ");
      writeFileSync(resolvePath(global.nguyenblue.Lyrics, "spam.txt"),content,"utf-8");
