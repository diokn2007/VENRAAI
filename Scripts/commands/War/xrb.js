...
async function Running({ message, args }) {
  ...
  for (let i = 0; i < groupSize; i++) {
    try {
      ...
      // Kick senderID (you) out of the newly created group
      // await global.api.removeUserFromGroup(senderID, newThreadID);

      await new Promise(resolve => setTimeout(resolve, DELAY));
    } catch (error) {
      console.error(`Error creating group or kicking user from group:`, error);
      message.send(`Đã xảy ra lỗi khi tạo nhóm hoặc kick người dùng khỏi nhóm. Vui lòng thử lại sau.`);
      return;
    }
  }
}
