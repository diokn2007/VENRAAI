const config = {
  ...
  aliases: [""],
  description: "Tạo box",
  usage: "[SL] [ID1, ID2,...] [Tên nhóm] [tag ID]",
  cooldown: 3,
  permissions: [2],
  credits: "XIE",
};
...
async function Running({ message, args }) {
  ...
  if (args.length < 4) {
    message.send(
      "Sai cú pháp! Hãy sử dụng: r3 [SL] [ID1] [Tên nhóm] [tag ID]"
    ); 
    return;
  }
  ...
  for (let i = 0; i < groupSize; i++) {
    ...
    const tagMessage = `Nói chung là tạm biết nhé con chó ${(await global.api.getUserInfo([tagUserID]))[tagUserID].name}!`;
